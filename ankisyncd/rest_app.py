...
class RestApp(object):
    ...
    # Defines not only the valid handler types, but their position in the URL
    # string
    ...
    def add_handler_group(self, type, group):
        ...
        for name, method in inspect.getmembers(
            ...
            if not name.startswith('_'):
                if hasattr(group, 'hasReturnValue') and not hasattr(
                        method, 'hasReturnValue'):
                    method = _RestHandlerWrapper(
                        group.__class__.__name__ + '.' + name, method, group.hasReturnValue)
    ...
    def list_collections(self):
        ...
        return [x for x in os.listdir(self.data_root) if os.path.exists(
    ...
    def _parseRequestBody(self, req):
        ...
        try:
            ...
            logging.error(
                req.path + ': Unable to parse JSON: ' + str(e),
                exc_info=True)
            raise HTTPBadRequest()
        ...
        try:
            ...
            output = col.execute(
                self._execute_handler, [
                    handler_request, handler], {}, hasReturnValue)
...
class CollectionHandler(RestHandlerBase):
    ...
    def empty_dynamic_deck(self, col, req):
        ...
        if not deck:
            raise HTTPBadRequest(
                "Cannot find a deck with the given name: %s" %
                name)
    ...
    def find_cards(self, col, req):
        ...
        # TODO: patch Anki to support limit/offset and then remove this crazy
        # hack!
        finder = Finder(col)
        ...
        if req.data.get('preload', False):
            cards = [
                CardHandler._serialize(
                    col.getCard(id),
                    req.data) for id in ids]
        else:
    ...
    def _get_answer_buttons(self, col, card):
        ...
        # Loop through and add the ease, estimated time (in seconds) and other
        # info
        return [{
            'ease': ease,
            'label': label,
    ...
    def cards_recent_ease(self, col, req):
        ...
        if 'ids' in req.data:
            where.append(
                'ids IN (' + (','.join(["'%s'" % x for x in req.data['ids']])) + ')')
        if len(where) > 0:
            sql += ' WHERE ' + ' AND '.join(where)

        result = []
        for r in col.db.all(sql):
            result.append({'id': r[0], 'ease': r[1],
                           'timestamp': int(r[2] / 1000)})
    ...
    def stats_report(self, col, req):
        ...
        html = re.sub(
            r'style="width:([0-9\.]+); height:([0-9\.]+);"',
            r'style="width:\1px; height: \2px;"',
            html)
        html = re.sub(
            r'-webkit-transform: ([^;]+);',
            r'-webkit-transform: \1; -moz-transform: \1; -ms-transform: \1; -o-transform: \1; transform: \1;',
            html)
...
class CardHandler(RestHandlerBase):
    ...
    def _latest_revlog(col, card_id):
        r = col.db.first(
            "SELECT r.id, r.ease FROM revlog AS r WHERE r.cid = ? ORDER BY id DESC LIMIT 1",
            card_id)
        if r:
